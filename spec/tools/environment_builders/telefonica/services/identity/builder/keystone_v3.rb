    ...
    module Identity
      class Builder
        class KeystoneV3
          ...
          def initialize(ems)
            @service = ems.connect(:service => "Identity", :telefonica_endpoint_type => 'adminURL')
          ...
          def find_or_create_projects
            @data.projects.each do |project|
              telefonica_project = (find(@service.projects.all(:domain_id => domain_id), project.slice(:name)) ||
                                   create_project(@service.projects, @projects, domain_id, project))
              @projects << telefonica_project
            end
          end
